...
public enum GeographicType {
    ...
    public static GeographicType getGeographicType(final Integer pType) {
        GeographicType aGeographicType = null;
        for (GeographicType aType : GeographicType.values()) {
            if (aType.getValue().equals(pType)) {
                aGeographicType = aType;
                break;
            }
        }
        return aGeographicType;
    }

    /**
     * Gives the Enumerated type for <code>GeographicType</code>.
     * <p>
     * 
     * @param pGeographicHierarchy The hierarchy of GeographicType.
     * 
     * @return the enumerated value of the <code>GeographicType</code>.<br>
     */
    public static GeographicType getGeographicTypeByHierarchy(final int pGeographicHierarchy) {
        GeographicType anAggregateType = null;

        for (final GeographicType aType : GeographicType.values()) {
            if (aType.getHierarchy() == pGeographicHierarchy) {
